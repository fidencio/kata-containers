...
func updateCDIDevices(ctx context.Context, ociSpec *specs.Spec, config *oci.RuntimeConfig) error {
	...
	for devName, res := range resolvers {
		...
		resp, err := invokeGRPC(ctx, true, res, req)
		if err != nil {
			return wrappedError(err)
		}
...
func cleanupCDIResolver(s *service, c *container) error {
	...
	for _, res := range resolvers {
		...
		_, err := invokeGRPC(ctx, false, res, req)
		if err != nil {
...
func invokeGRPC(ctx context.Context, allocate bool, res *oci.Resolver, req interface{}) (*pb.PhysicalDeviceResponse, error) {
	addr := res.Socket
	if addr == "" {
	...
	conn, err := getResolverConn(addr)
	if err != nil {
		return nil, wrappedError(err)
	}
	defer conn.Close()

	rc := pb.NewCDIResolverClient(conn)
...
func getResolverConn(addr string) (*grpc.ClientConn, error) {
	conn, err := grpc.Dial(
		addr,
		grpc.WithInsecure(),
