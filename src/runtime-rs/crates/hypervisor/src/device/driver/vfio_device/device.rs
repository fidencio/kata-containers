...
impl VfioDeviceModern {
    pub fn new(device_id: String, base: &VfioDeviceBase) -> Result<Self> {
        ...
        let device = if base.iommu_device_node.is_some() {
            discover_vfio_device(base.iommu_device_node.as_ref().unwrap())?
        } else { // Fallback to group device discovery
            discover_vfio_group_device(base.iommu_group_devnode.clone())?
        };
