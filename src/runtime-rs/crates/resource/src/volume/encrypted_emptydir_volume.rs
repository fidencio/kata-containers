...
fn write_volume_mount_info(volume_path: &str, info: &DirectVolumeMountInfo) -> Result<()> {
    ...
    fs::set_permissions(&file_path, fs::Permissions::from_mode(0o600))
        .with_context(|| format!("set permissions on {file_path:?}"))?;
    Ok(())
}
...
mod tests {
    ...
    fn make_mount(typ: Option<&str>, source: Option<&str>) -> oci::Mount {
        let mut m = oci::Mount::default();
        m.set_typ(typ.map(String::from));
        m.set_source(source.map(PathBuf::from));
        m.set_destination(PathBuf::from("/mnt/test"));
        m
    }

    // ── is_encrypted_emptydir_volume ──────────────────────────────────────

    #[rstest]
    #[case("shared-fs")]
    #[case("")]
    #[case("unknown")]
    fn wrong_emptydir_mode_returns_false(#[case] mode: &str) {
        let m = make_mount(
            Some("bind"),
            Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~empty-dir/vol"),
        );
        assert!(!is_encrypted_emptydir_volume(&m, mode));
    }
    ...
    #[case(Some("overlay"))]
    #[case(None)]
    fn non_bind_mount_type_returns_false(#[case] typ: Option<&str>) {
        let m = make_mount(
            typ,
            Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~empty-dir/vol"),
        );
        assert!(!is_encrypted_emptydir_volume(&m, "block-encrypted"));
    }

    #[rstest]
    #[case(Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~configmap/cm"))]
    #[case(Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~secret/sec"))]
    #[case(Some("/some/random/path"))]
    #[case(None)]
    fn non_emptydir_source_returns_false(#[case] source: Option<&str>) {
        let m = make_mount(Some("bind"), source);
        assert!(!is_encrypted_emptydir_volume(&m, "block-encrypted"));
    }

    #[test]
    fn valid_encrypted_emptydir() {
        let m = make_mount(
            Some("bind"),
            Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~empty-dir/vol"),
        );
    ...
    #[test]
    fn emptydir_path_without_subdir_returns_false() {
        let m = make_mount(
            Some("bind"),
            Some("/var/lib/kubelet/pods/uid/volumes/kubernetes.io~empty-dir"),
        );
        assert!(!is_encrypted_emptydir_volume(&m, "block-encrypted"));
    }
}
