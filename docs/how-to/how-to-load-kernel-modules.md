...
│    images]]             │           │ /dev/vdb ── /lib/modules/      │
│   path = "mlx5.img"     │──attach──▶│              kata-modules-0/   │
│   path = "custom.img"   │──attach──▶│ /dev/vdc ── /lib/modules/      │
│                         │           │              kata-modules-1/   │
└─────────────────────────┘           │                                │
                                      │ depmod -a  ──▶ modprobe ...    │
                                      └────────────────────────────────┘
...
```bash
# Collect modules into a staging directory
STAGING=$(mktemp -d)
...
2. **Runtime** creates `Storage` entries in the `CreateSandboxRequest`
   for each image, with mount points at `/lib/modules/kata-modules-0`,
   `/lib/modules/kata-modules-1`, etc.

3. **Agent** processes storages *before* loading kernel modules:
   - Mounts each modules volume read-only.
   - Writes `/etc/depmod.d/kata-modules.conf` with search directives
     pointing to the mounted directories.
   - Runs `depmod -a` to rebuild the module dependency database.
   - Proceeds to load any kernel modules specified in
     `CreateSandboxRequest.kernel_modules` via `modprobe`.
...
| `kernel-modules-images` | **Yes** | Built with `-x` (signs the shipped module images) |
