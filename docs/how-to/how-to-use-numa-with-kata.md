...
and creates a 1:1 guest-to-host mapping. To group multiple host nodes into
fewer guest nodes (e.g., on a 4-socket system), specify an explicit mapping:
...
2. **Builds guest topology**: Creates guest NUMA nodes with per-node memory
...
3. **Restructures SMP**: Sets `sockets = num_NUMA_nodes` and
   `cores = ceil(maxvcpus / num_NUMA_nodes)` so QEMU groups vCPUs by socket
   per NUMA node.

4. **Pins vCPUs** (when enabled): Each vCPU thread is pinned to a host CPU
   belonging to the same NUMA node.

5. **Validates VFIO devices**: Checks each cold-plugged device's host NUMA
   node against the guest topology and logs placement status.

6. **Translates cpuset.mems**: Converts host NUMA node IDs to guest node IDs
