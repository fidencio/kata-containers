...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let kernel_modules_images_enabled =
            env::var("KERNEL_MODULES_IMAGES_ENABLED").unwrap_or_else(|_| "false".to_string())
                == "true";
...
fn parse_kernel_modules_images() -> Result<HashMap<String, Vec<KernelModulesImageEntry>>> {
    ...
    for line in content.lines() {
        ...
        let verity_params = parts.get(2).map(|s| s.trim().to_string()).unwrap_or_default();
        let modules = parts
            .get(3)
            .map(|s| {
                s.split(',')
