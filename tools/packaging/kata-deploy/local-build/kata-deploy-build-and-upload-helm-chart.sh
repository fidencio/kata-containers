...
if [[ "${REGISTRY}" == *-ci ]]; then
	JOB_DISPATCHER_REGISTRY="${JOB_DISPATCHER_REGISTRY:-"${REGISTRY%-ci}-job-dispatcher-ci"}"
else
	JOB_DISPATCHER_REGISTRY="${JOB_DISPATCHER_REGISTRY:-"${REGISTRY}-job-dispatcher"}"
fi
...
yq eval ".job.dispatcherImage.reference = \"${JOB_DISPATCHER_REGISTRY}\" | .job.dispatcherImage.tag = \"${TAG}\"" -i "${tmp}/kata-deploy/values.yaml"
