...
if [[ "${REGISTRY}" == *-ci ]]; then
	default_job_dispatcher_registry="${REGISTRY%-ci}-job-dispatcher-ci"
else
	default_job_dispatcher_registry="${REGISTRY}-job-dispatcher"
fi
JOB_DISPATCHER_REGISTRY="${4:-${default_job_dispatcher_registry}}"
