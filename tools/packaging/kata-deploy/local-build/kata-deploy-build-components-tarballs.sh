...
# stage. Build it once and let each component package its own binary, so running
# both components locally does not compile the workspace twice.
build_rust_binaries() {
	if [[ ! -d "${rust_builder_out}" ]]; then
		docker buildx build \
			--target rust-builder \
