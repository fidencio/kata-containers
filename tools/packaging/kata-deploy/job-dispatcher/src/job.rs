...
pub fn job_name(prefix: &str, node: &str) -> String {
    ...
    let keep = 63usize.saturating_sub(hash.len() + 1);
...
mod tests {
    ...
    #[test]
    fn test_job_name_short() {
        assert_eq!(
            job_name("kata-deploy-install", "worker-0"),
            "kata-deploy-install-worker-0"
        );
    }
...
"#,
    ...
    #[test]
    fn test_interpret_status_complete() {
        let job = job_with_status("  conditions:\n    - type: Complete\n      status: \"True\"\n");
        assert_eq!(interpret_status(&job), JobOutcome::Succeeded);
    }

    #[test]
    fn test_interpret_status_failed() {
        let job = job_with_status("  conditions:\n    - type: Failed\n      status: \"True\"\n");
        assert_eq!(interpret_status(&job), JobOutcome::Failed);
    }

    #[test]
    fn test_interpret_status_running_when_condition_false() {
        let job = job_with_status("  conditions:\n    - type: Complete\n      status: \"False\"\n");
        assert_eq!(interpret_status(&job), JobOutcome::Running);
    }
    ...
    #[test]
    fn test_interpret_status_succeeded_counter_fallback() {
        let job = job_with_status("  succeeded: 1\n");
        assert_eq!(interpret_status(&job), JobOutcome::Succeeded);
    }
}
