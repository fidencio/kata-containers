...
use clap::Parser;
use job::{build_node_job, interpret_status, job_name, JobOutcome, OWNER_LABEL};
...
async fn main() -> Result<()> {
    ...
    run_fanout(&jobs, &template, &nodes, &args, parallelism, owner.as_ref()).await
}
...
async fn run_fanout(
    ...
    let owner_selector = format!("{}={}", OWNER_LABEL, args.name_prefix);
    ...
    while !queue.is_empty() || !in_flight.is_empty() {
        ...
        while in_flight.len() < parallelism {
            ...
            let name = job_name(&args.name_prefix, node);
            let node_job = build_node_job(template, &name, node, &args.name_prefix, owner);
            match jobs.create(&post, &node_job).await {
                Ok(_) => info!("created job {name} (node {node})"),
                // Adopt a Job left over from a previous run (idempotent re-run).
                Err(kube::Error::Api(e)) if e.code == 409 => {
                    info!("job {name} (node {node}) already exists, adopting it")
                }
                Err(e) => {
    ...
    if !failed.is_empty() {
        failed.sort();
        failed.dedup();
        bail!("{} node(s) failed: {}", failed.len(), failed.join(", "));
    }
